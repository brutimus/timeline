...
function timeline_chart() {
  ...
  function my(selection) {
    ...
    function load_image(config, points, data){
        ...
        map_width = svgNode.getBBox().width;
        map_height = svgNode.getBBox().height;
        ...
        $.each(points, function(index, el) {
            if (el.svg_id) {
                ...
                obj.attr('transform', 'translate(' + el.off_x + ',' + el.off_y + ')')
                    .attr('class', 'sprite')
                    // .datum(el, function(x){return x.id});
            } else {
    ...
    function zoomToPoint(point){
        zoom_state = 'point',
        scale = 3,
        translate = [(map_width * .25) - scale * point.svg_bb.x, map_height / 2 - scale * point.svg_bb.y];
    ...
    function zoomToPoints(points){
        ...
        // $.map(points, function(d) {bounds.expandBy(d.svg_bb)});
        // console.log(bounds, bounds.width(), bounds.height())

        // scale = Math.min(.9 / Math.max(
        //     bounds.width() / map_width,
        //     bounds.height() / map_height), 3);
        // console.log(map_width, scale, bounds.minX, bounds.maxX)
        // translate = [
        //     map_width / 2 - scale * (bounds.minX + bounds.maxX),
        //     map_height / 2 - scale * (bounds.minY + bounds.maxY)];
        ...
        //     x = (bbx1 + bbx2) / 2,
        //     y = (bby1 + bby2) / 2,
        //     scale = Math.min(.9 / Math.max(dx / map_width, dy / map_height), 3),
        //     translate = [map_width / 2 - scale * x, map_height / 2 - scale * y];
        //     // console.log(map_width, scale, x)
        //     console.log(translate, scale)
        ...
        var n = content.select('#Pins'),
            transform = getTransform(n, 2.0);
        ...
        return content
            .transition()
            .duration(750)
            .call(zoom.translate(transform.translate).scale(transform.scale).event);
    }
    ...
    function changeSelection(d, i){
        ...
        var sprites = content.select('#Pins').selectAll('.sprite')
            .data(selected_points, function(p) { return p ? p.name : this.id; });
